...
const montantAchat = parseInt(prompt("entrer le montant des achats"));
// 2. Demandez à l'utilisateur d'entrer le nombre d'enfants dans la famille.
const nbEnfant = parseInt(prompt("entrer  nombre d’enfants dans la famille"));
...
if ((montantAchat>50) || (nbEnfant>=3)) {
    reduction = 0.1;
}else {
    reduction = 0.05;
}
console.log(`le nouveaux prix  =  ${montantAchat}-${reduction*100}% = ${montantAchat - (montantAchat*reduction)} `);